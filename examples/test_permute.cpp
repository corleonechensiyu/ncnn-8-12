...
{	
	...
	ncnn::Net ocr;
	ocr.load_param("test_blstm.param");
	ocr.load_model("test_blstm.bin");
	ncnn::Mat in = ncnn::Mat::from_pixels_resize(bgr.data,ncnn::Mat::PIXEL_BGR,bgr.cols,bgr.rows,64,32);
	
	const float mean_vals[3]={183.f,185.f,210.f};
	...
	ex.extract("reshape_permute",reshape);
	ex.extract("rlstm1_input",reverse1);
	ex.extract("lstm1",lstm1);
	ex.extract("rlstm1-output",lstm2);
	ex.extract("rlstm1",reverse2);
...
int main()
{
	cv::Mat m = cv::imread("1577436089755hl.jpg",1);
    if(m.empty())
    {        
